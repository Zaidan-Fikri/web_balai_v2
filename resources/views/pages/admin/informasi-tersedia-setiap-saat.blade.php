...
@section('title', 'Admin Informasi Tersedia Setiap Saat')
...
@section('content')
        ...
        <div class="panel full-card">
            ...
            <div class="table-wrap">
                <table class="item-table">
                            ...
                            <td>
                                <div class="action-group">
                                    <button type="button" class="btn-action update js-update-btn"
                                        data-update-url="{{ route('admin.informasi-tersedia-setiap-saat.update', $item->id) }}"
                                        data-tahun="{{ $item->tahun }}"
                                        data-judul="{{ $item->judul }}"
                                        data-pdf-url="{{ $pdfUrl }}"
                                        data-pdf-name="{{ $pdfName }}">Update</button>
                                    <form method="POST" action="{{ route('admin.informasi-tersedia-setiap-saat.destroy', $item->id) }}" class="js-delete-form">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="4">Belum ada dokumen informasi tersedia setiap saat.</td></tr>
                    @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div class="popup-overlay" id="createOverlay" aria-hidden="true">
        <div class="popup-card">
            ...
            <form method="POST" action="{{ route('admin.informasi-tersedia-setiap-saat.store') }}" enctype="multipart/form-data">
                @csrf
