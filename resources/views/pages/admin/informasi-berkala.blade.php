...
@section('title', 'Admin Informasi Berkala')
...
@section('content')
            ...
            <div class="table-wrap">
                <table class="item-table">
                    ...
                    @forelse ($items as $item)
                        ...
                        @endphp
                        <tr>
                            <td>{{ $kategoriLabels[$item->kategori] ?? $item->kategori }}</td>
                            <td>{{ $item->tahun }}</td>
                            <td>{{ $item->judul }}</td>
                            ...
                            <td>
                                <div class="action-group">
                                    <button type="button" class="btn-action update js-update-btn"
                                        data-update-url="{{ route('admin.informasi-berkala.update', $item->id) }}"
                                        data-kategori="{{ $item->kategori }}"
                                        data-tahun="{{ $item->tahun }}"
                                        data-judul="{{ $item->judul }}"
                                        data-pdf-url="{{ $pdfUrl }}"
                                        data-pdf-name="{{ $pdfName }}">Update</button>
                                    <form method="POST" action="{{ route('admin.informasi-berkala.destroy', $item->id) }}" class="js-delete-form">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="5">Belum ada dokumen informasi berkala.</td></tr>
                    @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div class="popup-overlay" id="createOverlay" aria-hidden="true">
        <div class="popup-card">
            ...
            <form method="POST" action="{{ route('admin.informasi-berkala.store') }}" enctype="multipart/form-data">
                @csrf
                <select class="popup-input" name="kategori" required>
    ...
    <div class="popup-overlay" id="updateOverlay" aria-hidden="true">
        <div class="popup-card">
            ...
            <form method="POST" id="updateForm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <select class="popup-input" id="updateKategori" name="kategori" required>
                    @foreach ($kategoriLabels as $key => $label)
...
(function () {
    ...
    document.querySelectorAll('.js-update-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            document.getElementById('updateKategori').value = btn.dataset.kategori;
    ...
    document.querySelectorAll('[data-close-overlay]').forEach(function (btn) {
        btn.addEventListener('click', function () {
            var target = document.getElementById(btn.dataset.closeOverlay);
            if (target) { target.setAttribute('aria-hidden', 'true'); target.style.display = 'none'; }
        });
    });
